...
SELECT YEAR(data) AS ano, MONTH(data) AS mes, AVG(contagem) AS media_mensal, nomeLocal
FROM ( select t.dataAbert data, l.id local, l.nome nomeLocal, count(*) as contagem
       from tarefa t
       join locais l on l.id = t.localFK
       group by t.id) subconsulta GROUP BY YEAR(data), MONTH(data);
